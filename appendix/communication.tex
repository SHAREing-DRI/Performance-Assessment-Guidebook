\chapter{Communication with code owner}
\label{appendix:communication}

There are many times throughout an assessment where direct communication may be required. For now we list the area of `Existing Optimisations', which is a tricky scenario to properly include in an assessment.

\section{Existing optimisations}
\label{section:communication:optimisations}
...
They have been written first and foremost with functional requirements (``the science'') in mind.
The HPC came later.
...
Therefore, we always have to keep in mind that performance has been seen as a second-class citizen in most development projects. 
By the time a project has reached a certain maturity and people suffer from performance flaws, they will ask for a performance assessment (and then want to be told that they have already done the heavy lifting or that their science is so challenging and completely different to everything else that it is very hard to get good performance).
