\chapter{Performance assessment---kickstart workflow}
% \setcounter{section}{0}
\label{chapter:performance-assessment}
...
We employ few very simple principles:

\begin{enumerate}
  \item We study the performance characteristics for \emph{different machine part/aspects separately}. 
    It is natural to distinguish inter-node, intra-node, core and GPU performance (Figure~\ref{figure:performance-assessment:machine-parts}). 
  \item We use a simple \emph{blackbox performance metric} per dimension to find out first if there are issues or not.
  \item Per dimension, we then \emph{break down performance recursively into subrubrics} and dig deeper and deeper into the code.
\end{enumerate}


\begin{figure}[htb]
 \begin{center}
  %\includegraphics[width=0.55\textwidth]{performance-assessment-overview.pdf}
  \includegraphics[width=0.55\textwidth]{images/performance_diag.pdf}
 \end{center}
 \caption{
  The five machine dimensions along which performance measurements can be
  discriminated.
  \label{figure:performance-assessment:machine-parts}
 }
\end{figure}


\noindent
The approach is natural yet does not come along without shortcomings.
If there is a performance issue with a code that affects multiple dimensions (e.g.~a poor GPU utilisation that materialises in bad load balancing as well), it might be blurred, lost or make us study unneccessary metrics.
When we dig down recursively into finer and finer metrics along a metric tree, we have to rely on the fact that each step of the process tracks the right quantities of interest which flag the next finer detail to look at.

As we analyse the four dimensions of performance characteristics independently, the order does not really make a difference.
In theory.
In practice, we have to start somewhere.
I personally made the impression that it is usually favourable to start with the node-level analysis (intra-node): 
